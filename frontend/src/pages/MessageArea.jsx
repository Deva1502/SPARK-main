...
return ()=>socket?.off("newMessage")
  ...
  return (
    <div className='w-full h-[100vh] bg-green-100 relative'>
      
      <div className=' w-full flex items-center gap-[15px] px-[20px]  py-[10px] fixed top-0 z-[100] bg-green-100 '>
...
<div className='w-[40px] h-[40px] border-2 border-black rounded-full cursor-pointer overflow-hidden' onClick={()=>navigate(`/profile/${selectedUser.userName}`)}>
      ...
      <div className='w-full h-[80%] pt-[100px]  px-[40px] flex flex-col gap-[50px] overflow-auto bg-green-200'>
{messages && messages?.map((mess,index)=>
  mess.sender==userData._id?<SenderMessage message={mess}/>:<ReceiverMessage message={mess}/>
)}
      </div>

<div className='w-full h-[80px] fixed bottom-0 flex justify-center items-center bg-green-100 z-[100]'>
<form className='w-[90%] max-w-[800px] h-[80%] rounded-full bg-green-200 flex items-center gap-[10px] px-[20px] relative' onSubmit={handleSendMessage}>
  ...
  <input type="text" placeholder='Message' className='w-full h-full px-[20px] text-[18px] text-black outline-0' onChange={(e)=>setInput(e.target.value)} value={input}/>
  <div onClick={()=>imageInput.current.click()}><LuImage className='w-[28px] h-[28px] text-black'/></div>
  {(input || frontendImage) &&  <button className='w-[60px] h-[40px] rounded-full bg-gradient-to-br from-[#9500ff] to-[#ff0095] flex items-center justify-center cursor-pointer'><IoMdSend className='w-[25px] h-[25px] text-black'/></button>}
